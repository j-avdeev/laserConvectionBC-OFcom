...
Acknowledgement
    Financial support by the Austrian Federal Government
    (in particular from Bundesministerium fuer Verkehr, Innovation und
    Technologie and Bundesministerium fuer Wirtschaft, Familie und Jugend)
    represented by Oesterreichische Forschungsfoerderungsgesellschaft mbH and
    the Styrian and the Tyrolean Provincial Government, represented by
    Steirische Wirtschaftsfoerderungsgesellschaft mbH and Standortagentur
    Tirol, within the framework of the COMET Funding Programme is
    ...
    debug_(false)
{
    ...
    //- List of content in the dictionary
    const wordList& contentOfTable = dict.toc();

    //- Bool for source dict check
    bool sourceDictFound = false;

    forAll(contentOfTable, c)
    {
        //- Subdict found -> new LASER source
        if (dict.isDict(contentOfTable[c]))
        {
            ...
            //- Change bool (means at least 1 source exist)
            sourceDictFound = true;

            //- Add name of source dict
            ...
            //- Otherwise the LASER source will be active till the heating
            //  time is passed
            else
            {
                ...
                forAll(contentOfSourceDict, csd)
                {
                    if (sourceDict.isDict(contentOfSourceDict[csd]))
                    {
                        ...
                        else if (motionMode_[nSources_-1] == "linear")
                        {
                            ...
                            //- Calculate the accumulation of time that is
                            //  for the different linear parts | + length
                            for
                            (
                                int i=0;
                            ...
                            )
                            {
                                ...
                                //- Time to reach the end point p_(n+1)
                                //  from t = 0
                                if (i == 0)
                                {
                                ...
                                else
                                {
                                    ...
                                    //const scalar prevLength =
                                    //    lengthAcc_[nSources_-1][i-1];
        ...
        if (motion_[m] && !endPoint_[m])
        {
            ...
            if (t > startMotion_[m] && !centerUpdated_[m])
            {
                ...
                else if (motionMode_[m] == "linear")
                {
                    ...
                    /*Info<< "Time to reach: " << timeAcc_[m][id] << " | " << t << endl;
                    Info<< "Source center: " << sourceCenters_[m] << endl;
                    Info<< "Start:         " << lMPoints_[m][id] << endl;
                    Info<< "Target:        " << lMPoints_[m][id+1] << endl;
                    Info<< "Length:        " << length << endl;
                    Info<< "Time needed:   " << timeFromP1toP2 << endl << endl;
                    Info<< "Reduced time   " << timeReduce << endl << endl;
                    Info<< "Ratio:         " << ratio << endl;
                    Info<< "Vector:        " << p1ToP2 << endl << endl;
                    Info<< "SC Calc:       " << lMPoints_[m][id] << " + "
                        << p1ToP2 * ratio << endl;*/
                }
            }
        }
